...
% ---------------- Figure 3 : Differential‑thrust sensitivity -------------
% Select the operating point that maximises range
MR         = (Range == max(Range));
Drag_best  = Drag(MR);
V_best     = V_flight(MR);

% Generate thrust split vector (0 = all bottom, 1 = all top)
T_top_percent = (0:0.05:1)';
T_bot_percent = 1 - T_top_percent;

% Convert percentage split to absolute thrust values on each rotor pair
T_top = T_top_percent .* Drag_best;
T_bot = T_bot_percent .* Drag_best;

m_motors = floor(n_motors/2);

% Interpolate power draw & rotor speed for each thrust level
power_top = interp1(Thrust_N.*m_motors, Power_W.*2, T_top , "pchip","extrap");
power_bot = interp1(Thrust_N.*m_motors, Power_W.*2, T_bot , "pchip","extrap");
RPM_top   = interp1(Thrust_N.*m_motors, RPM,        T_top , "pchip","extrap");
tho_top   = interp1(Thrust_N.*m_motors, Throttle,   T_top , "pchip","extrap");

% Endurance & range for each split
Capacity_best      = battery_capacity(battery_mass == MassBattery(MR));
power_flight       = power_top + power_bot;
diffT_time_flight  = Capacity_best ./ power_flight;      % [s]
diffT_Range        = diffT_time_flight .* V_best;        % [m]
diffT_time_flight  = diffT_time_flight ./ 60;            % [min]

% Store for further analysis if needed
diff_velocity_data = table(T_top_percent, tho_top, T_top, ...
                           diffT_Range, diffT_time_flight, ...
                           power_top, RPM_top);

% Plot the effect of thrust split on mission performance
figure;
yyaxis left
plot(T_top_percent, diffT_Range/1000, ...
     'LineWidth', 3, ...
     'Color', [0, 0.4470, 0.7410]);         % Blue = range
ylabel('Range (km)');

yyaxis right
plot(T_top_percent, diffT_time_flight, ...
     'LineWidth', 3, ...
     'Color', [0.8500, 0.3250, 0.0980]);    % Orange = endurance
ylabel('Time of flight (min)');
ylim([min(diffT_time_flight)-1, max(diffT_time_flight)+7]);

grid on;

xlabel('Top Motors Thrust Share');
title('Differential Thrust Flight Performance');
legend('Range (km)', 'Time of flight (min)', 'Location','best');
